...
const allServices = [...site.services.surgical, ...site.services.general];

function getService(slug: string) {
  const item = allServices.find((service) => service.slug === slug);
  const page = site.servicePages.find((service) => service.slug === slug);
  return item && page ? { item, page } : null;
}

export function generateStaticParams() {
  return site.servicePages.map((service) => ({ slug: service.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!service) return {};
  return {
    title: `${service.item.title} — ${site.clinic.name}`,
    ...
    <>
      <ServiceHero
        ...
        breadcrumb={[
          ...
          { label: t.breadcrumbLabel, href: "/xizmatlar" },
          { label: item.title },
        ]}
        heading={item.title}
        intro={page.intro}
      ...
      {/* FAQ: accordion-free list on ivory */}
      <section className="bg-ivory py-24 md:py-32">
        <Container>
          <Reveal>
